...
def getDailyForcastedReadingsInRange(db, zip_code, start_date, end_date):
    '''start_date and end_date are inclusive
    '''
    if type(start_date) is str:
        start_date = datetime.strptime(start_date, "%Y-%m-%d").date()
    ...
    names = ['recorded_on','zip_code','hightemp','precipitation','probability_of_precip','one_day_high','three_day_high']
